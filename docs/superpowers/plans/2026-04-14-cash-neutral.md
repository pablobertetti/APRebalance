...
**Goal:** Ensure rebalance trades never require injecting unintentional cash — buys never exceed sells by more than `cashAdjustment`.

**Architecture:** Single post-processing block inserted in `rebalancer.js` after all raw trades are generated. Computes deficit = buys − sells − cashAdjustment; if positive, removes shares one-by-one from the highest-priced buy trade until deficit ≤ 0. `targetSharesMap` is kept in sync so `deployedValue` stays correct.
